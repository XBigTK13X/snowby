...
const playlist = {
    ...
    loadPlaylist: function(playlist, mode = 'replace') {
        return new Promise((resolve, reject) => {
            ...
            if (!['replace', 'append'].includes(mode)) {
                return reject(
                    this.errorHandler.errorMessage(1, 'loadPlaylist()', [playlist, mode], null, {
                            ...
                            if (message.length > 0) {
                                ...
                                if ('event' in message) {
                                    ...
                                    // when the track has changed we don't need a seek event
                                        ...
                                        this.getProperty(`playlist/0/filename`).then(filename => {
    ...
    next: function(mode = 'weak') {
        return new Promise((resolve, reject) => {
            ...
            if (!['weak', 'force'].includes(mode)) {
                return reject(
                    this.errorHandler.errorMessage(1, 'next()', [mode], null, {
                ...
                this.getPlaylistPosition1().then(position => {
                    // geht the playlist size
                    this.getPlaylistSize().then(size => {
                        ...
                        // continue the skipping process
                            ...
                            return this.getProperty(`playlist/${this.observed['playlist-pos'] + 1}/filename`).then(nextTrackFile => {
                                // socket to observe the command
                                const observeSocket = net.Socket()
                                observeSocket.connect({ path: this.options.socket }, () => {
                                    // send the skip command to mpv
                                    this.socket
                                        .command('playlist-next', [mode])
                                        .then(() => {
                                            // timeout
                                            let timeout = 0
                                            // check if the file was started
    ...
    prev: function(mode = 'weak') {
        return new Promise((resolve, reject) => {
            ...
            if (!['weak', 'force'].includes(mode)) {
                return reject(
                    this.errorHandler.errorMessage(1, mode, 'prev()', null, {
                ...
                this.getPlaylistPosition1().then(position => {
                    ...
                    // continue the skipping process
                        ...
                        return this.getProperty(`playlist/${this.observed['playlist-pos'] - 1}/filename`).then(prevTrackFile => {
                            return new Promise((resolve, reject) => {
                                // socket to observe the command
                                const observeSocket = net.Socket()
                                observeSocket.connect({ path: this.options.socket }, () => {
                                    // send the prev command to mpv
                                    this.socket.command('playlist-prev', [mode]).then(() => {
                                        // timeout
                                        let timeout = 0
                                        // check if the file was started
                                        let started = false
