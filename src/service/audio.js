...
const keepAwake = () => {
    ...
    if (settings.keepAudioDeviceAwake) {
        ...
        audioProcess = spawn(settings.mpvExePath, [`${settings.inaudibleWavPath}`, `--loop`, `--vo=null`, `--no-config`, `--gapless-audio=yes`, '--volume=1']) // DEBUG ->, `--log-file=${util.appPath('/bin/mpv/mpv-audio-keep-awake.log')}`])
    }
}
